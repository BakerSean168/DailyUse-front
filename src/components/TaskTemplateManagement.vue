<template>
    <div class="task-template-management">
        <div class="flex justify-between items-center mb-4">
            <div class="template-filters">
                <button v-for="status in statusFilters" :key="status.value" class="filter-btn"
                    ...
                    <Icon :icon="status.icon" width="20" height="20" />
                    {{ status.label }}
                    <span class="count">{{ getTemplateCountByStatus(status.value) }}</span>
                </button>
            </div>
            <div>
                <button class="btn btn-primary" @click="startCreateTaskTemplate">
                    创建新模板
                </button>
            </div>
        </div>
            ...
            <div v-for="template in filteredTemplates" :key="template.id" class="template-card">
                ...
                <div class="template-meta">
                    ...
                    <div class="repeat-pattern">
                        <Icon icon="mdi:repeat" width="16" height="16" />
...
const statusFilters = [
    ...
    { label: '未开始', value: 'upcoming', icon: 'mdi:clock-outline' },
